...
import static cpm.umcs.music.ListenerAccount.Persistence.hasSong;
...
public class ListenerAccount extends Account {
    ...
    public Playlist createPlaylist(List<Integer> songIds) throws SQLException, NotEnoughCreditsException {
        ...
        for(var id: songIds) {
            ...
            var optionalSong = Song.Persistance.read(id);
    ...
    private void buySong(int songId) throws SQLException, NotEnoughCreditsException {
            if (Persistence.getCredits(getId()) >= 1){
                Persistence.addSong(getId(), songId);
                Persistence.addCredits(getId(), -1);
            }
            else throw new NotEnoughCreditsException("Not enough credits");
    }
    ...
    public static class Persistence {
        public static void init() throws SQLException {
            Account.Persistence.init();
            {
                ...
                PreparedStatement statement = cpm.umcs.database.DatabaseConnection.getConnection("connection").prepareStatement(sql);
